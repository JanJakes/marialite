...
build_inside_container() {
  ...
  local cmake_args=(
    ...
    -DMYLITE_DISABLE_BINLOG_SYSVARS=ON
    -DMYLITE_DISABLE_BINLOG_CACHE_DIR=ON
    -DMYLITE_DISABLE_BINLOG_OBJECT_INIT=ON
    -DMYLITE_DISABLE_RPL_GTID_STATE=ON
    -DMYLITE_DISABLE_RPL_UTILITY_SERVER=ON
    -DMYLITE_DISABLE_TC_LOG_MMAP=ON
    -DMYLITE_DISABLE_RPL_FILTER=ON
    -DMYLITE_DISABLE_CRYPT_FUNCTION=ON
    -DMYLITE_DISABLE_DES_FUNCTIONS=ON
    -DMYLITE_DISABLE_DYNAMIC_COLUMNS=ON
    -DMYLITE_DISABLE_EMBEDDED_CLIENT_FALLBACKS=ON
    -DMYLITE_DISABLE_EMBEDDED_DEFAULT_FILES=ON
    -DMYLITE_DISABLE_EMBEDDED_MYSQL_C_API=ON
    -DMYLITE_DISABLE_AUTH_PROTOCOL=ON
    -DMYLITE_DISABLE_SERVER_ACCOUNT_SQL=ON
    -DMYLITE_DISABLE_KDF_FUNCTION=ON
    -DMYLITE_DISABLE_DYNAMIC_PLUGIN_LOADING=ON
    -DMYLITE_DISABLE_EXTRA_LOCALES=ON
    -DMYLITE_DISABLE_FULLTEXT_MATCH=ON
    -DMYLITE_DISABLE_LOAD_DATA=ON
    -DMYLITE_DISABLE_ZLIB_COMPRESSION=ON
    -DMYLITE_DISABLE_GEOMETRY_TYPE=ON
    -DMYLITE_DISABLE_GENERAL1400_COLLATIONS=ON
    -DMYLITE_DISABLE_MYSQL500_COLLATIONS=ON
    -DMYLITE_DISABLE_UCA_COLLATIONS=ON
    -DMYLITE_DISABLE_UNWIND_TABLES=ON
    -DMYLITE_DISABLE_SQL_CRYPTO_FUNCTIONS=ON
    -DMYLITE_DISABLE_SERVER_ENCRYPTION=ON
    -DMYLITE_DISABLE_OPENSSL_DIGESTS=ON
    -DMYLITE_DISABLE_OPTIMIZER_TRACE=ON
    -DMYLITE_DISABLE_UDF_RUNTIME=ON
    -DMYLITE_DISABLE_WINDOW_FUNCTIONS=ON
    -DMYLITE_DISABLE_ORACLE_FUNCTIONS=ON
    -DMYLITE_DISABLE_ORACLE_PARSER=ON
    -DMYLITE_DISABLE_EVENT_PARSE_DATA=ON
    -DMYLITE_DISABLE_GIS_FUNCTIONS=ON
    -DMYLITE_DISABLE_FOREIGN_SERVER_CACHE=ON
    -DMYLITE_DISABLE_HELP_COMMAND=ON
    -DMYLITE_DISABLE_JSON_FUNCTIONS=ON
    -DMYLITE_DISABLE_JSON_SCHEMA_VALID=ON
    -DMYLITE_DISABLE_JSON_TABLE=ON
    -DMYLITE_DISABLE_JSON_TYPE=ON
    -DMYLITE_DISABLE_SQL_DIGEST=ON
    -DMYLITE_DISABLE_SQL_DIAGNOSTICS_STATEMENTS=ON
    -DMYLITE_DISABLE_SQL_EMBEDDED_EXCEPTIONS=ON
    -DMYLITE_DISABLE_PLSQL_CURSOR_ATTRIBUTES=ON
    -DMYLITE_DISABLE_STORED_FUNCTION_LOOKUP=ON
    -DMYLITE_DISABLE_STORED_PROGRAM_RUNTIME=ON
    -DMYLITE_DISABLE_FULL_ERROR_MESSAGES=ON
    -DMYLITE_DISABLE_SYSTEM_VERSIONING=ON
    -DMYLITE_DISABLE_EXPLAIN_RUNTIME=ON
    -DMYLITE_DISABLE_PROXY_PROTOCOL=ON
    -DMYLITE_DISABLE_TRIGGER_RUNTIME=ON
    -DMYLITE_DISABLE_VIEW_RUNTIME=ON
    -DMYLITE_DISABLE_XA_TRANSACTIONS=ON
    ...
    -DMYLITE_DISABLE_PERSISTENT_STATISTICS=ON
    -DMYLITE_DISABLE_PROCEDURE_ANALYSE=ON
    -DMYLITE_DISABLE_PROCESSLIST_METADATA=ON
    -DMYLITE_DISABLE_SELECT_PROCEDURE_RUNTIME=ON
    -DMYLITE_DISABLE_SHOW_CREATE_RUNTIME=ON
    -DMYLITE_DISABLE_SHOW_STATIC_INFO=ON
    -DMYLITE_DISABLE_STATUS_METADATA=ON
    -DMYLITE_DISABLE_SYSVAR_HELP_TEXT=ON
    -DMYLITE_DISABLE_OPTION_HELP_TEXT=ON
    -DMYLITE_DISABLE_QUERY_LOGS=ON
    -DMYLITE_DISABLE_QUERY_CACHE=ON
    -DMYLITE_DISABLE_REGEX_FUNCTIONS=ON
    -DMYLITE_DISABLE_ROUTINE_INFORMATION_SCHEMA=ON
    -DMYLITE_DISABLE_SFORMAT_FUNCTION=ON
    -DMYLITE_DISABLE_SELECT_OUTFILE=ON
    -DMYLITE_DISABLE_SERVER_UTILITY_FUNCTIONS=ON
    -DMYLITE_DISABLE_PREPARED_STATEMENT_API=ON
    -DMYLITE_DISABLE_SQL_HANDLER_COMMAND=ON
    -DMYLITE_DISABLE_SQL_PREPARE_COMMANDS=ON
    -DMYLITE_DISABLE_SQL_SEQUENCE=ON
    -DMYLITE_DISABLE_SPATIAL_CORE=ON
    -DMYLITE_DISABLE_TABLE_ADMIN=ON
    -DMYLITE_DISABLE_TIME_ZONE_TABLES=ON
    -DMYLITE_DISABLE_TPOOL_RUNTIME=ON
    -DMYLITE_DISABLE_VECTOR_FUNCTIONS=ON
    -DMYLITE_DISABLE_VECTOR_TYPE=ON
    -DMYLITE_DISABLE_VIO_SSL=ON
...
write_build_report() {
...

  {
    ...
    grep -E "^(AWS_SDK_EXTERNAL_PROJECT|BUILD_CONFIG|CMAKE_((C|CXX)_(FLAGS_MINSIZEREL|VISIBILITY_PRESET)|(EXE|MODULE|SHARED)_LINKER_FLAGS|VISIBILITY_INLINES_HIDDEN)|DEFAULT_COLLATION|DISABLE_SHARED|ENABLED_PROFILING|FEATURE_SET|MYLITE_(DISABLE|ENABLE)_[A-Z0-9_]+|PLUGIN_[A-Z0-9_]+|UPDATE_SUBMODULES|USE_ARIA_FOR_TMP_TABLES|WITH_[A-Z0-9_]+|WITHOUT_DYNAMIC_PLUGINS):" \
