...
- `CMAKE_EXE_LINKER_FLAGS=-fuse-ld=lld -Wl,-O2 -Wl,-z,pack-relative-relocs -Wl,--pack-dyn-relocs=relr -Wl,--no-eh-frame-hdr -Wl,--gc-sections -Wl,--icf=all`
- `CMAKE_MODULE_LINKER_FLAGS=-fuse-ld=lld -Wl,-O2 -Wl,-z,pack-relative-relocs -Wl,--pack-dyn-relocs=relr -Wl,--no-eh-frame-hdr -Wl,--gc-sections -Wl,--icf=all`
- `CMAKE_SHARED_LINKER_FLAGS=-fuse-ld=lld -Wl,-O2 -Wl,-z,pack-relative-relocs -Wl,--pack-dyn-relocs=relr -Wl,--no-eh-frame-hdr -Wl,--gc-sections -Wl,--icf=all`
...
- `MYLITE_DISABLE_SFORMAT_FUNCTION=ON`
...
- `MYLITE_DISABLE_SQL_EMBEDDED_EXCEPTIONS=ON`
...
- `MYLITE_DISABLE_SHOW_CREATE_RUNTIME=ON`
...
- `MYLITE_DISABLE_TPOOL_RUNTIME=ON`
...
- `MYLITE_DISABLE_BINLOG_SYSVARS=ON`
- `MYLITE_DISABLE_BINLOG_CACHE_DIR=ON`
- `MYLITE_DISABLE_BINLOG_OBJECT_INIT=ON`
...
`no-prepared-api-size-profile`, `show-create-size-profile`,
`tpool-wait-stub-size-profile`, and
...
handler, the retained JSON data-type alias, and the fmtlib-backed `SFORMAT()`
convenience function, disable MariaDB statement
...
`SHOW CONTRIBUTORS`, and `SHOW PRIVILEGES` metadata surfaces, omit
`SHOW CREATE` runtime and formatting from the aggressive profile, replace
inherited tpool wait notifications with no-op embedded hooks, bypass embedded
option-file loading after the required `--no-defaults` startup argument,
replace the process-list row producers with unsupported or empty embedded behavior, and
...
MySQL 5.0 utf8mb3/ucs2 collation implementation, and omit disabled
binary-log, GTID-binlog, relay-log, and replication system-variable
registration while retaining common harmless compatibility variables such as
`sql_log_bin`, `server_id`, `last_gtid`, and read-only `encrypt_binlog`, and
skip inherited binlog cache directory setup and `mysql_bin_log` object
instrumentation/init/cleanup in the no-binlog embedded profile, and reject
server user, role, grant, revoke, `SHOW GRANTS`, and `SHOW CREATE USER` SQL
explicitly instead of inheriting silent embedded no-access-check success.

Important compatibility note: this current stack is an aggressive size-floor
measurement stack. Several earlier SQL-surface removals in it, including the
`SHOW CREATE` runtime and prepared-statement API experiments, should not be
treated as product-default decisions without a separate compatibility review.
The newer embedded/server-only cuts, including the binlog sysvar, cache-dir,
and object-init slices, are measured on top of that stack because it is the
current size-research branch.
...
| `show-create-size-profile` after no prepared API | 25,389,516 | -18,015,916 | 4,552,192 | -14,779,712 | Passes current smokes and harness; rejects `SHOW CREATE` runtime variants and compiles out the heavy CREATE-statement formatting bodies |
| `tpool-wait-stub-size-profile` after SHOW CREATE | 25,391,070 | -18,014,362 | 4,551,888 | -14,780,016 | Passes current smokes and harness; replaces inherited tpool wait notification runtime with no-op embedded hooks, reducing linked size by 304 bytes while growing the merged static archive by 1,554 bytes |
| `embedded-default-files-size-profile` after tpool wait stubs | 25,391,134 | -18,014,298 | 4,546,760 | -14,785,144 | Passes current smokes and harness; bypasses embedded option-file loading after the required `--no-defaults` argument, removes `my_load_defaults()` and `get_defaults_options()` from the linked runtime, and grows the merged archive by 64 bytes |
| `auth-protocol-size-profile` after embedded default files | 25,371,140 | -18,034,292 | 4,538,104 | -14,793,800 | Passes current smokes and harness; omits network password-auth plugins and rejects `COM_CHANGE_USER` in the embedded minsize dispatcher while preserving MyLite's local open path |
| `binlog-sysvar-size-profile` after auth protocol | 25,281,364 | -18,124,068 | 4,527,880 | -14,804,024 | Passes current smokes and harness; omits disabled binlog, GTID-binlog, relay-log, and replication system-variable registration while keeping common harmless compatibility variables |
| `binlog-cache-dir-size-profile` after binlog sysvars | 25,281,276 | -18,124,156 | 4,526,352 | -14,805,552 | Passes current smokes and harness; skips inherited `#binlog_cache_files` setup in the embedded no-binlog profile |
| `binlog-object-init-size-profile` after binlog cache dir | 25,280,988 | -18,124,444 | 4,525,904 | -14,806,000 | Passes current smokes and harness; skips `mysql_bin_log` instrumentation setup, pthread-object init, and cleanup in the embedded no-binlog profile |
| `direct-mylite-dispatch-size-profile` after binlog object init | 25,283,578 | -18,121,854 | 4,524,384 | -14,807,520 | Passes current smokes and harness; routes `mylite_exec()` and `mylite_warning()` through a MyLite-owned direct embedded result adapter while still reusing the existing embedded `MYSQL *` session |
...
The `show-create-size-profile` then rejected `SHOW CREATE` runtime variants
and compiled out the heavy CREATE-statement formatting bodies from
`sql_show.cc`. On top of `no-prepared-api-size-profile`, it reduced the static
archive by another 100,150 bytes and the stripped linked smoke by another
14,496 bytes when both before/after binaries were stripped with GNU
`strip --strip-unneeded`. The open-close smoke verifies `SHOW CREATE TABLE`,
`SHOW CREATE DATABASE`, `SHOW CREATE SERVER`, `SHOW CREATE TRIGGER`,
`SHOW CREATE EVENT`, `SHOW CREATE PROCEDURE`, `SHOW CREATE PACKAGE`, and
`SHOW CREATE PACKAGE BODY` all report
`ER_NOT_SUPPORTED_YET`; linked symbol checks show `show_create_view`,
`show_create_sequence`, `show_create_trigger_impl`, and `get_trigger_table`
are absent. This is an aggressive introspection compatibility tradeoff, not a
default recommendation for every embedding.

The `tpool-wait-stub-size-profile` then replaced the inherited tpool wait
notification object with no-op embedded hooks for `tpool_wait_begin()` and
`tpool_wait_end()`. On top of `show-create-size-profile`, it grew the merged
static archive by 1,554 bytes because the stub is now part of
`libmariadbd.a`, but reduced `libmylite.a` by 8 bytes, the unstripped linked
smoke by 440 bytes, and the stripped linked smoke by 304 bytes. The open/close
smoke verifies no inherited `tpool::` runtime symbols are linked. This is a
valid embedded-shape cleanup, but the size impact is negligible.

The `embedded-default-files-size-profile` then bypassed
`load_defaults("my", ...)` in the aggressive embedded runtime when startup is
already controlled by the required `--no-defaults` argument. On top of
`tpool-wait-stub-size-profile`, it grew the merged static archive by 64 bytes
because the existing argv-copy helper is now retained, but reduced the
unstripped linked smoke by 6,104 bytes and the stripped linked smoke by
5,128 bytes. The open/close smoke verifies `my_load_defaults()`,
`get_defaults_options()`, and
`search_default_file()` are not linked. This is a clean embedded-shape win for
linked artifacts and does not change SQL behavior.

The `auth-protocol-size-profile` then removed MariaDB's built-in network
password-authentication plugin descriptors and plugin VIO handshake helpers
from the aggressive embedded profile. On top of
`embedded-default-files-size-profile`, it reduced the static archive by another
19,994 bytes, the unstripped linked smoke by 10,576 bytes, and the stripped
linked smoke by 8,656 bytes. The open/close smoke verifies
`builtin_maria_mysql_password_plugin`, native/old auth handlers,
`do_auth_once`, server handshake packet helpers, and `server_mpvio_*` helpers
are not linked. This does not remove SQL user-name parsing, `CURRENT_USER()`,
SQL password functions, or MyLite's local in-process open path.

The `binlog-sysvar-size-profile` then omitted system-variable registration for
disabled binary-log, GTID-binlog, relay-log, and replication behavior in the
aggressive embedded profile. On top of `auth-protocol-size-profile`, it reduced
the static archive by another 89,776 bytes, the unstripped linked smoke by
28,320 bytes, and the stripped linked smoke by 10,224 bytes. The linked smoke
no longer retains representative disabled variable names such as
`binlog_format`, `binlog_row_image`, `gtid_binlog_state`, `gtid_domain_id`,
`pseudo_slave_mode`, `replicate_do_db`, `slave_parallel_threads`, or
`sync_binlog`. The first attempt also removed `encrypt_binlog`, but the
open/close smoke caught that as an ordinary read-only disabled variable that
clients can inspect, so it remains available. This is a small but clean
embedded-shape win because it removes knobs for files and replication behavior
that the current minsize profile cannot create or manage.

The `binlog-cache-dir-size-profile` then skipped inherited
`#binlog_cache_files` directory setup in the embedded no-binlog profile. On top
of `binlog-sysvar-size-profile`, it reduced the static archive by 88 bytes, the
unstripped linked smoke by 1,904 bytes, and the stripped linked smoke by
1,528 bytes. The open/close smoke verifies `init_binlog_cache_dir()` and
`binlog_cache_dir` are no longer linked. This is a tiny size win, but it also
removes a server-style filesystem probe/delete step from startup.

The `binlog-object-init-size-profile` then skipped `mysql_bin_log`
instrumentation-key setup, pthread-object initialization, and cleanup in the
embedded no-binlog profile. On top of `binlog-cache-dir-size-profile`, it
reduced the static archive by 288 bytes, the unstripped linked smoke by
768 bytes, and the stripped linked smoke by 448 bytes. The linked `llvm-size`
decimal total increased by 28 bytes because `.text` dropped by 420 bytes while
measured `bss` grew by 448 bytes. The open/close smoke verifies
`MYSQL_BIN_LOG::init_pthread_objects()` and `MYSQL_BIN_LOG::cleanup()` are no
longer linked. This is an embedded startup cleanup, not a material size lever.

The `direct-mylite-dispatch-size-profile` then routed `mylite_exec()` and
`mylite_warning()` through a MyLite-owned direct embedded result adapter in
`lib_sql.cc`. This Stage 1 bridge still keeps the existing embedded `MYSQL *`
session and MariaDB `MYSQL_DATA` row capture, so it is not the final
client-C-API removal. On top of `binlog-object-init-size-profile`, it grew the
static archive by 2,590 bytes and `libmylite.a` by 550 bytes, but reduced the
unstripped open-close smoke by 1,664 bytes, the stripped open-close smoke by
1,520 bytes, and the `size` decimal total by 4,140 bytes. The implementation
mirrors `emb_read_rows()` row-list termination, copies result strings before
calling the public MyLite callback, and preserves `mylite_changes()`,
`mylite_last_insert_id()`, warning lookup, MariaDB errno, SQLSTATE, and error
message behavior in the open/close smoke. This is valid groundwork for removing
obsolete internal client C API roots later; by itself, it is a tiny linked-size
win rather than a meaningful production-size lever.

The `sformat-function-size-profile` then removed MariaDB's fmtlib-backed
`SFORMAT()` convenience string formatter from the aggressive minsize profile.
On top of `direct-mylite-dispatch-size-profile`, it reduced the static archive
by 291,688 bytes, the unstripped open-close smoke by 113,408 bytes, the
stripped open-close smoke by 72,280 bytes, and the `size` decimal total by
69,638 bytes. The open/close smoke verifies ordinary numeric `FORMAT()` still
works and `SFORMAT()` now fails explicitly with `ER_SP_DOES_NOT_EXIST`
(`FUNCTION SFORMAT does not exist`). Linked symbol checks show no remaining
`fmt::` symbols in `mylite-open-close-smoke`.

The `sql-embedded-no-exceptions-profile` then compiled only the retained
`sql_embedded` target with `-fno-exceptions`. On top of
`sformat-function-size-profile`, it reduced the static archive by
2,554,764 bytes, the unstripped open-close smoke by 464,000 bytes, the
stripped open-close smoke by 458,256 bytes, and the `size` decimal total by
458,692 bytes. The linked `.eh_frame` section fell from 426,808 bytes to
20,640 bytes, and `.gcc_except_table` fell from 33,036 bytes to 6,756 bytes.
The CMake option is guarded by `MYLITE_DISABLE_SFORMAT_FUNCTION` because
`SFORMAT()` was the retained SQL target's known C++ `catch` path. First-party
`libmylite` and `ha_mylite` code are not compiled with `-fno-exceptions`, so
their `std::bad_alloc` diagnostics remain available.

The `server-account-sql-profile` then fixed the inherited embedded
no-access-check behavior for server users and grants. It is not a size win:
the static archive grew by 576 bytes, the stripped open-close smoke grew by
1,712 bytes, the minimal linked executable grew by 192 bytes, and the
PHP-shaped shared probe grew by 208 bytes. The value is correctness: `CREATE
...
The `lld-o2-link-profile` then added `-Wl,-O2` to executable, module, and
shared lld flags. This is a small link-only win: it leaves the static
`libmariadbd.a` unchanged, reduces the stripped open-close smoke by
5,120 bytes, reduces the stripped minimal executable by 4,352 bytes, and
...
| Omit `SHOW CREATE` runtime formatting | 0.095 MiB archive, 0.014 MiB stripped linked beyond no prepared API | Medium SQL introspection compatibility | Applied as aggressive size attempt | Current smokes and harness pass; this removes useful schema-dump introspection, so it is a size floor lever rather than an obvious default |
| Replace tpool wait notifications with no-op embedded hooks | +0.001 MiB archive, 0.0003 MiB stripped linked beyond SHOW CREATE | Low embedded-runtime risk | Applied as embedded-shape size attempt | Current smokes and harness pass; no SQL compatibility change, but the linked-size win is only 304 bytes |
| Bypass embedded option-file loading after `--no-defaults` | +0.001 MiB archive, 0.005 MiB stripped linked beyond tpool wait stubs | Low embedded-runtime risk | Applied as embedded-shape size attempt | Current smokes and harness pass; MyLite uses explicit startup args and must not read host option files, but this is not generic MariaDB embedded C API behavior |
| Omit network auth protocol plugins | 0.019 MiB archive, 0.008 MiB stripped linked beyond embedded default files | Low/medium embedded C API compatibility | Applied as embedded-shape size attempt | Current smokes and harness pass; MyLite does not expose daemon login or `COM_CHANGE_USER`, while SQL user-name parsing and local open remain intact |
...
| Compile `sql_embedded` without C++ exceptions | 2.44 MiB archive, 0.44 MiB stripped linked beyond SFORMAT | Medium fork/build risk | Applied as aggressive embedded-build attempt | Current smokes and harness pass; first-party MyLite API/storage exception handling remains enabled, but future upstream imports must recheck retained SQL target exception use |
...
4. Keep lld `-O2` in the aggressive minsize linker flags. It is a small,
   low-risk linked-artifact win and preserves build-id plus RELRO.
5. Keep section GC for the most aggressive linked-runtime profile while the
   static archive growth remains an explicit tradeoff.
6. Keep the `JSON_SCHEMA_VALID()` omission in the minsize profile unless JSON
   Schema `CHECK` constraints become a compatibility target.
7. Keep the query-cache omission in the minsize profile. It is server-global
   cache state, reports unavailable through MariaDB's `have_query_cache`
   surface, and has low value in the embedded default.
8. Keep the Oracle compatibility alias omission in the aggressive minsize
...
9. Keep the server-utility function omission in the aggressive minsize profile.
...
10. Keep the UCA-collation omission only for the most aggressive size profile
...
11. Keep the regex/PCRE2 omission only for the most aggressive size profile
   unless product compatibility accepts losing `REGEXP`, `RLIKE`, and
   `REGEXP_*()` SQL functions.
12. Keep the command-level binlog/replication source omission as a small archive
   cleanup, but do not treat it as meaningful runtime-size work.
13. Keep the no-binlog-core no-op layer and guarded event/GTID source omission
...
14. Keep the MyISAM admin omission in the aggressive minsize profile while
   user `ENGINE=MyISAM` remains rejected.
15. Keep the MyISAM full-text omission in the aggressive minsize profile while
   MyLite full-text indexes remain unsupported.
16. Keep the MyISAM RTREE omission in the aggressive minsize profile while
   MyLite spatial indexes remain unsupported.
17. Keep the spatial-core omission in the aggressive minsize profile while
   MyLite GEOMETRY storage, SPATIAL indexes, and GIS functions remain
   unsupported.
18. Keep the SQL sequence omission in the aggressive minsize profile if MyLite
   does not need MariaDB sequence objects as a compatibility target.
19. Keep the GEOMETRY type implementation omission in the aggressive minsize
   profile while MyLite treats GEOMETRY and SPATIAL DDL as unsupported.
20. Keep the `general1400` collation and extended casefold omission only in the
   aggressive UCA-disabled minsize profile.
21. Keep lld `--icf=all` only in the aggressive linked-runtime profile unless
   release testing proves no address-identity issues in target integrations.
22. Keep VIO TLS omitted in the aggressive embedded profile while MyLite has no
   network listener or remote client login path. This removes `libssl.so.3`
   from the runtime dependency set.
23. Keep the legacy `ENCRYPT()` omission in the aggressive embedded profile.
   It removes `libcrypt.so.1` from the runtime dependency set; retained crypto
   functions still cover current smoke needs.
24. Keep the zlib compression omission in the aggressive embedded profile. It
   removes `libz.so.1` from the runtime dependency set while retaining `CRC32()`
   and rejecting compressed-column DDL explicitly.
25. Keep the legacy DES omission in the aggressive embedded profile. The size
...
32. Keep the OpenSSL-free digest wrapper profile for the aggressive embedded
...
33. Keep the backup-stage omission in the aggressive embedded profile. External
...
34. Keep `JSON_TABLE` omitted only in the most aggressive size profile. The
...
35. Keep the foreign-server metadata cache omitted in the aggressive embedded
   profile while foreign-server metadata remains unsupported.
36. Keep proxy protocol support omitted in the aggressive embedded profile.
   It is a network-listener feature, and the current MyLite runtime has no
   network accept path.
37. Keep full event parser data omitted in the aggressive embedded profile
...
38. Keep XA transactions omitted in the aggressive embedded profile while
...
39. Keep trigger sidecar loading and execution omitted in the aggressive
   ...
   sidecars.
40. Keep view sidecar loading and execution omitted in the aggressive embedded
...
41. Keep table-admin maintenance omitted in the aggressive embedded profile.
...
42. Keep persistent engine-independent statistics omitted in the aggressive
...
43. Keep the generic `SELECT ... PROCEDURE` runtime omitted in the aggressive
...
44. Keep the non-`en_US` locale table omitted only in the most aggressive size
...
45. Keep `LOAD DATA` / `LOAD XML` omitted in the aggressive embedded profile
...
46. Keep a stripped linked smoke binary size in the build report so regressions
   are visible.
47. Keep hidden default C/C++ visibility in the aggressive minsize profile.
...
48. Keep EXPLAIN/ANALYZE plan-output runtime omitted only in the most
...
49. Keep the retained `VECTOR` type handler omitted in the aggressive embedded
   ...
   compatibility plan.
50. Keep ordinary JSON SQL functions omitted only in the most aggressive size
...
51. Keep the `JSON` data-type alias omitted only in the most aggressive size
...
52. Keep SQL statement digest normalization omitted in the aggressive embedded
...
53. Keep SQL diagnostics statements omitted only in the most aggressive size
...
54. Keep system-versioning runtime omitted in the aggressive embedded profile
...
55. Keep row-replication conversion utilities omitted in the aggressive
...
56. Keep dynamic columns omitted in the aggressive embedded profile. The win is
   ...
   requirement.
57. Keep routine Information Schema scans omitted while stored routine DDL
...
58. Keep static `SHOW AUTHORS`, `SHOW CONTRIBUTORS`, and `SHOW PRIVILEGES`
...
59. Keep process-list row rendering omitted in the aggressive embedded profile.
...
60. Keep stored-function lookup omitted in the aggressive embedded profile
...
61. Keep PL/SQL cursor attribute item runtime omitted in the aggressive
...
62. Keep status metadata publication omitted in the aggressive embedded
...
63. Keep system-variable help text omitted in the aggressive embedded profile.
...
64. Keep command-line option help text omitted in the aggressive embedded
...
65. Keep general and slow query logging omitted in the aggressive embedded
...
66. Keep the stored-program runtime objects omitted in the aggressive embedded
...
67. Keep the compact server error-message catalog in the aggressive embedded
...
68. Keep linked `.eh_frame_hdr` omitted only in the aggressive minsize profile.
...
69. Keep SQL `MATCH ... AGAINST` omitted only in the aggressive minsize
...
70. Keep SQL `HANDLER` commands omitted only in the aggressive minsize profile.
   ...
   handles if needed.
71. Keep `SELECT ... INTO OUTFILE/DUMPFILE` omitted in the aggressive embedded
...
72. Keep MyISAM temp-spill omitted in the aggressive embedded profile. The
...
73. Keep the disabled server-option row trim in the aggressive embedded
...
74. Keep disabled binlog/replication sysvar registration omitted in the
   aggressive embedded profile. The win is small, but it removes variables for
   disabled binlog, relay-log, GTID-binlog, and replication file surfaces while
   keeping common harmless compatibility variables.
75. Keep inherited binlog cache-dir setup omitted in the aggressive embedded
   profile. The win is tiny, but startup should not probe or delete
   `#binlog_cache_files` when binlogging is disabled.
76. Keep inherited binlog object init/cleanup omitted in the aggressive
   embedded profile. The win is tiny, but `mysql_bin_log` synchronization setup
   is not needed when the no-binlog core keeps that object closed and inert.
77. Keep legacy MySQL 5.0 collations omitted only in the aggressive minsize
   profile. The win is tiny but real, and imported metadata that names those
   legacy collations will fail.
78. Keep embedded client fallbacks omitted in the aggressive minsize profile.
...
79. Keep SQL-language prepared-statement commands omitted in the aggressive
...
80. Treat `no-prepared-api-size-profile` as a lower-bound experiment, not as
